...
static uint8_t usbio_buf[1024];
static struct usbio_data *iodata = (struct usbio_data *)usbio_buf;
...
static int Card_GetSpecs(uint16_t *pagesize, uint16_t *blocksize, int32_t *cardsize, uint8_t *flags)
{
	...
	*flags = iodata->mc_data[2];
	*pagesize = read_le_uint16(&iodata->mc_data[3]);
	*blocksize = read_le_uint16(&iodata->mc_data[5]);
	*cardsize = read_le_uint32(&iodata->mc_data[7]);
...
static int Card_ReadPageWithEcc(int32_t page, uint8_t *pagebuf, uint8_t *eccbuf)
{
...
static int Card_FindFree(int reserve)
{
	...
	struct MCDevInfo *mcdi = (struct MCDevInfo *)&mcio_devinfo;
	struct MCCacheEntry *mce1, *mce2 = 0;
	...
	for (r1 = 0; r1 < clusters_per_block; r1++) {
		...
		for (r2 = 0; r2 < pages_per_cluster; r2++) {
			mcio_pagedata[(r1 * ((pages_per_cluster << 16) >> 16)) + r2] = \
				(uint8_t *)(mce->cl_data + (r2 * pagesize));
		}
	}

	/* Erase the block where data must be written */
	r = Card_EraseBlock(value1, (uint8_t **)mcio_pagedata, (uint8_t *)mcio_eccdata);
	if (r != sceMcResSucceed)
		return r;
...
static int Card_FileOpen(char *filename, int flags)
{
	...
	strncpy_s((char *)fse2->name, sizeof((char *)fse2->name), p, 32);
...
int mcio_mcCreateCrossLinkedFile(char *real_filepath, char *dummy_filepath)
{
	...
	strncpy_s(dummy_filename, sizeof(dummy_filename), pfse->name, 32);
...
int mcio_mcWriteBlock(int block, uint8_t **pagedata, uint8_t **eccbuf)
{
	...
	{
		r = Card_WritePageData(block*blocksize + i, pagedata[i], eccbuf[i]);
		if (r != sceMcResSucceed)
			return r;
	}
	return sceMcResSucceed;
